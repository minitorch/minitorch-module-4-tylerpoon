...
def tile(input: Tensor, kernel: Tuple[int, int]) -> Tuple[Tensor, int, int]:
    ...
    new_tensor = new_tensor.permute(0, 1, 2, 4, 3, 5)
    new_tensor = new_tensor.contiguous().view(batch, channel, new_height, new_width, kh * kw)
...
def avgpool2d(input: Tensor, kernel: Tuple[int, int]) -> Tensor:
    ...
    return new_input.mean(dim=-1).view(batch, channel, new_height, new_width)
...
def maxpool2d(input: Tensor, kernel: Tuple[int, int]) -> Tensor:
    ...
    batch, channel, height, width = input.shape
    new_input, new_height, new_width = tile(input, kernel)
    return max(new_input, -1).view(batch, channel, new_height, new_width)
